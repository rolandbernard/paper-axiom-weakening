...
do
    ...
    do
        ...
        if ! [ -e $run/repair-remove.log ]
        then
            if ! timeout 5m \
                systemd-run --scope -p MemoryMax=10G --user \
                java -cp target/ontologyutils.jar -Xms9G www.ontologyutils.apps.RepairRemoval \
