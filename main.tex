...
% mention other gentle repair approaches
% mention previous work in axiom weakening
% briefly describe what we have done
The classical approach to repairing inconsistent ontologies is to identify and remove problematic axioms so that the ontology becomes consistent (e.g., \cite{ScCo03,kalyanpur2005debugging,kalyanpur2006repairing,BaPS07}). However, deletion whole axioms can lead to the removal of more information than is strictly necessary to regain consistency. More gentle repair methods using \emph{axiom weakening} have therefore been proposed in the literature \cite{du2014practical,AMAI-2018,DBLP:conf/kr/BaaderKNP18,troquard2018repairing,confalonieri2020towards}. In \cite{DBLP:conf/kr/BaaderKNP18} the authors show general theoretical results for repair using axiom weakening, and propose a concrete weakening relation for the $\mathcal{EL}$ description logics. In \cite{troquard2018repairing} axiom weakening using refinement operators has been described for \ALC and experimentally evaluated, showing that axiom weakening is able to retain more information than deletion. In \cite{confalonieri2020towards} the axiom weakening has been extended to include many aspects of \SROIQ, notably omitting however the weakening of RBox axioms. The authors of \cite{confalonieri2020towards} further show that the proposed repair by iterated weakening almost surely terminates.

The main goal of this paper is to extend further extend axiom weakening to handle all \SROIQ axiom types including simple and complex RIAs. Additionally, by implementing the proposed refinement and weakening operators we are able to perform experimental evaluations also on ontologies using the more expressive features of \SROIQ.
...
where $r \in N_R$ is a role name, $n > 1$ and $R$, $S$, $S_1, \cdots, S_n$ are roles such that $S \preceq R$, $S_i \preceq R$, and $R \not\preceq S_i$ for $i = 1, \dots, n$.\footnote{The definitions for simple roles and regularity used here aligns with the global restrictions in OWL 2 DL \cite{motik2009owl}.}
